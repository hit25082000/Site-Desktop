    ...
    try {
      if (isSignUp) {
        ...
        if (data?.session) {
          ...
          setSuccessMessage('Cadastro realizado com sucesso! Verifique seu e-mail para confirmação se necessário.');
          setEmail('');
          setPassword('');
          setConfirmPassword('');
        }
      ...
      setError(err.message || 'Erro ao realizar a operação. Verifique suas credenciais.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md bg-white border border-neutral-200/80 rounded-[2.5rem] shadow-xl p-8 relative z-10">
        ...
        <div className="flex bg-neutral-100 p-1 rounded-2xl mb-6">
          <button
            ...
            className={`flex-1 py-2.5 text-sm font-medium rounded-xl transition font-geist ${!isSignUp ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500 hover:text-neutral-900'}`}
          >
            Entrar
          </button>
          <button
        ...
        {/* Success Alert */}
        {successMessage && (
          <div className="flex items-center gap-3 p-4 mb-6 rounded-2xl bg-emerald-50 border border-emerald-100 text-emerald-800 text-sm font-geist">
            <WarningCircle className="w-5 h-5 shrink-0 text-emerald-600 rotate-180" weight="light" />
        ...
        <form onSubmit={handleAuth} className="space-y-5">
          ...
          >
            <span>{loading ? (isSignUp ? 'Cadastrando...' : 'Entrando...') : (isSignUp ? 'Criar Conta' : 'Acessar Painel')}</span>
